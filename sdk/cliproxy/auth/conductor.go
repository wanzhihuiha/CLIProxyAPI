...
func (m *Manager) Remove(ctx context.Context, id string) {
	...
	m.queueCodexQuotaRefreshUnschedule(id)
...
func (m *Manager) executeStreamMixedOnce(ctx context.Context, providers []string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, maxRetryCredentials int) (*cliproxyexecutor.StreamResult, error) {
	...
	for {
		...
		streamResult, errStream := m.executeStreamWithModelPool(execCtx, executor, auth, provider, execReq, opts, routeModel, models, pooled, activeLease, stickyLease)
		if errStream != nil {
			if errCtx := execCtx.Err(); errCtx != nil {
				releaseActiveSessionLease(activeLease)
				return nil, errCtx
